...
public class ConsoleFormatter
{
    ...
    public static ArrayList<String> getClusterNames(final Collection<Cluster> clusters, ArrayList<String> list)
    {
    	list.clear();
    	for (final Cluster cluster : clusters)
        {
    		list.add(cluster.getLabel());
    		System.out.println(cluster.getLabel());
//    		System.out.print("(");
            getDocumentIds(cluster);
//            System.out.println(")");   		
        }
    	return list;
    }
    
    private static ArrayList<String> getDocumentIds(Cluster cluster)
    {
    	ArrayList docList = new ArrayList();
    	for (final Document document : cluster.getDocuments())
        {
//	        System.out.print(document.getTitle()+" ");
//	        docList.add(document.getStringId());	        
	        docList.add(document.getTitle());
        }
    	
    	CiteSeerXCluster.clusterDocs.put(cluster.getLabel().trim(),docList);
    	
    	return docList;
    }
    
    public static void displayClustersNameOnly(final Collection<Cluster> clusters)
    {
    	displayClustersNameOnly(clusters, Integer.MAX_VALUE);
    }

    public static void displayClustersNameOnly(final Collection<Cluster> clusters,
            int maxNumberOfDocumentsToShow)
    {
    	displayClustersNameOnly(clusters, maxNumberOfDocumentsToShow,
        ...
        int maxNumberOfDocumentsToShow, ClusterDetailsFormatter clusterDetailsFormatter)
    {
        ...
        for (final Cluster cluster : clusters)
        {
        	displayClusterNameOnly(0, "" + clusterNumber++, cluster, maxNumberOfDocumentsToShow,
                clusterDetailsFormatter);
        }
    }
    
    private static void displayClusterNameOnly(final int level, String tag, Cluster cluster,
            int maxNumberOfDocumentsToShow, ClusterDetailsFormatter clusterDetailsFormatter)
    {
